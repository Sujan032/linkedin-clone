...
export const imageUploadHandler = async (
  ...
  switch (type) {
    case "video":
      if (
        !ACCEPTED_VIDEO_FORMATS.some((format) => format.includes(inputFileExec))
      ) {
        Swal.fire({
          title: "Invalid Video Format",
          text: `Please select video format of ${ACCEPTED_VIDEO_FORMATS.map(
            (format) => format + " "
          )}`,
          icon: "warning",
          width: "400px",
      ...
      if (fileSize > MAX_VIDEO_UPLOAD_SIZE) {
        Swal.fire({
          ...
          text: `Please select a video less than ${MAX_VIDEO_UPLOAD_SIZE}MB file size`,
          icon: "warning",
          width: "400px",
        });
        e.target.value = "";
    ...
    case "image":
      if (
        !ACCEPTED_IMAGE_FORMATS.some((format) => format.includes(inputFileExec))
      ) {
        Swal.fire({
          title: "Invalid Image Format",
          text: `Please select an image format of ${ACCEPTED_IMAGE_FORMATS.map(
            (format) => format + " "
          )}`,
          icon: "warning",
          width: "400px",
      ...
      if (fileSize > MAX_IMAGE_UPLOAD_SIZE) {
        Swal.fire({
          ...
          text: `Please select an image less than ${MAX_IMAGE_UPLOAD_SIZE}MB file size`,
          icon: "warning",
          width: "400px",
        });
        e.target.value = "";
